...
export const SEEN_KEY = 'sponsorable-sync-revision';
...
async function writeOnce(s: Session, replaceUnreadable: boolean): Promise<boolean> {
  ...
  switch (result.kind) {
    ...
    case 'offline':
      status({
        mode: 'browser-only',
        notice: 'The server stopped. Edits are kept in this browser, and reach the file when you restart `sponsorable serve` and reload.',
      });
      return false;
  }
}
