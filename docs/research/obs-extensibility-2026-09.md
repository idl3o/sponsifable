...
Status on 2026-09-12: the third item is built. The server owns `~/.sponsorable/workspace.json` and serves it at `/api/workspace`, with compare-and-swap writes. The first item is built in part: `/overlay.html?deal=<id>` draws the disclosure and the sponsor's name for a won deal, with no sponsor asset yet.

The second is built as `sponsorable log <deal>`, in `python/sponsorable/onair.py`, and tested against a fake OBS that can drop an activation signal the way studio mode did. It has **not** been run against real OBS: the probe's question is still open, which is why the poll carries the log and the events are only prompts. Starting it from the app, and the signed delivery report that reads its output, come next.
