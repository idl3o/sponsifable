...
- `pipx install 'sponsorable[seal]'` and `sponsorable setup`.
...
5. **Decode each one.** `sponsorable verify downloaded.jpg --started <today>`. The claim itself will fail the ordering check, because the ad started after the seal but on the same day. That is expected. What you are recording is whether the serial decoded.
